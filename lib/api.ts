...
export async function loadEncryptedNote(siteName: string) {
  ...
  return handleResponse(response) as Promise<{ payload: EncryptedPayload | null; updatedAt?: number }>;
}

export async function checkSiteExists(siteName: string) {
  const response = await fetch(`/api/check?site=${encodeURIComponent(siteName)}`);
  return handleResponse(response) as Promise<{ exists: boolean; updatedAt?: number }>;
}
